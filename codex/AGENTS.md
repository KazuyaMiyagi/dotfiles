...
Markdown ファイル（`.md` / `.markdown`）を変更した場合、コミット前に以下を必ず実行してエラー・警告がないことを確認する。エディタ保存時に efm-langserver 経由で走る Markdown 用リンター（`efm-langserver/config.yaml` の `languages.markdown`）と同じツールなので、CLI の結果とエディタの指摘が一致する。
...
- efm-langserver は引数の Markdown ファイルに対してこの 3 つを常に走らせる設定になっている。ルール／設定を変えるときは `efm-langserver/config.yaml` の `markdownlint-cli2` / `textlint` / `prettier` と `~/.config/textlint/textlintrc` を更新し、CLI とエディタで挙動を揃える
